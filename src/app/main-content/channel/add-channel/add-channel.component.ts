...
export class AddChannelComponent implements OnInit {
  ...
  /**
   * Shows the overlay visibility based on the channel name.
  ...
  showOverlay() {
    if (this.channelData.name !== '') {
      ...
      if (this.channelNameExists == false) {
        ...
        this.unhideOverlay = true;
      }
    } else {
      this.channelNameEmpty = true;
      this.unhideOverlay = false;
    }
  }

  /**
   * Closes the overlay visibility
   */
  closeOverlay() {
   ...
   */
  handleOutsideClick() {
    if (this.overlayVisible && this.unhideOverlay) {
      this.unhideOverlay = false;
    } else {
      this.overlayVisible = false;
    }
